...
| `Tab` | 循环单一语言过滤（`All` → 语言 1 → 语言 2 → … → `All`）。 |
...
> **已知限制：** 在 `INPUT` 模式下再次按 `/` 会被快捷键处理程序吞掉，而不是输入到过滤框中。因此无法过滤包含 `/` 的名称（例如路径片段）。
...
| `q` | **当前行为：** 退出应用。**建议：** 与其他覆盖层模式保持一致，改为关闭弹窗。 |
| `Ctrl+C` | 退出应用。 |
...
> 预览底部提示目前只写了 `q`；实际上 `q` 和 `Esc` 都可以关闭预览。
...
1. **`SELECT_LANG` 中 `q` 的行为不一致。** 在 `INPUT`、`SEARCH` 和 `PREVIEW` 中，`q` 被当作输入或关闭操作处理；而在 `SELECT_LANG` 中，`q` 当前会直接退出应用。建议改为关闭弹窗，与其他覆盖层保持一致。
2. **`INPUT` 模式下无法输入 `/`。** 由于 `/` 是进入 `INPUT` 的全局快捷键，当已经在过滤模式下再次按 `/` 时会被吞掉，而不是输入到过滤框中。因此无法过滤包含 `/` 的名称（例如路径片段）。
3. **存在未使用的按键常量。** `render/bindings.go` 中定义了 `left` 和 `right` 常量，但没有任何地方使用；建议删除或补充实现。
4. **预览帮助文本不完整。** 预览底部只提到 `q` 可关闭，应该同时说明 `Esc` 也可以关闭。
5. **`Tab` 的帮助描述不准确。** 当前帮助写的是 “Toggle language filter”，实际行为是循环切换语言；建议改为 “Cycle language filter”。
6. **终端快捷键冲突风险。** `Ctrl+L`（清屏）、`Ctrl+W`（删除前一个词）、`Ctrl+P`（历史搜索）在某些终端或 Shell 中可能被拦截。如果这些键不生效，用户可能需要调整终端配置。
7. **`PREVIEW` 模式下 `home` / `end` 不生效。** 预览使用的 viewport 没有绑定这两个键，因此会被静默忽略。建议要么为预览 viewport 显式添加绑定，要么保持当前文档口径仅列出实际生效的键。
