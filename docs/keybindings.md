...
| `+` / `=` | （Tree 模式）展开当前目录下的所有子目录。 |
| `-` | （Tree 模式）折叠当前目录下的所有子目录。 |
| `O` | （Tree 模式）递归展开光标所在目录的整个子树。 |
| `X` | （Tree 模式）递归折叠光标所在目录的整个子树。 |
| `/` | 进入 `INPUT` 名称过滤模式。 |
...
鼠标方面，左键双击某个块钻取进入对应目录，右键单击则返回上级目录。
...
├── 编辑: e
├── Tree 展开/折叠: +/= (全部展开), - (全部折叠), O (展开光标子树), X (折叠光标子树)
├── 帮助: ?
└── 退出: q, Ctrl+C
...
在支持鼠标事件的终端中，可以使用以下鼠标操作：

| 操作 | 功能 |
|------|------|
| 滚轮 | 移动光标 / 滚动预览和语言列表 |
| 左键单击 | 选中某一行或语言选择弹窗中的项 |
| 左键单击表头 | 按该列排序；再次单击当前排序列可切换升序/降序 |
| 左键双击 | 进入目录、展开/折叠目录或打开文件预览 |
| 右键单击（Treemap） | 返回上级目录 |
| 在浮层外单击 | 关闭文件预览、语言选择或饼图浮层 |
