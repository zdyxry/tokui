...
4. **终端快捷键冲突风险。** `Ctrl+L`（清屏）、`Ctrl+W`（删除前一个词）、`Ctrl+P`（历史搜索）在某些终端或 Shell 中可能被拦截。如果这些键不生效，用户可能需要调整终端配置。
5. **`PREVIEW` 模式下 `home` / `end` 不生效。** 预览使用的 viewport 没有绑定这两个键，因此会被静默忽略。建议要么为预览 viewport 显式添加绑定，要么保持当前文档口径仅列出实际生效的键。
